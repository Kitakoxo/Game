...
namespace FontList
{
	constexpr const wchar_t* FontPath[] = {
	L"Arial Unicode MS",
	L"onryou",
	L"HolidayMDJP05",
	L"GN-キルゴUかなNA",
...
struct FontData
{
	...
	bool enableOutline;							// 縁取りを有効にするか
	D2D1_COLOR_F outlineColor;					// 縁取りの色
	float outlineThickness;						// 縁取りの太さ（ピクセル単位）
...
public:
	...
	// コンストラクタ
	// 第1引数：フォント設定
	DirectWriteCustomFont(FontData* set);

	//デストラクタ
	~DirectWriteCustomFont();

	HRESULT RecreateRenderTarget(IDXGISwapChain* swapChain);
	void ReleaseRenderTarget();
	static void ReleaseAllRenderTargets();
	static void RecreateAllRenderTargets(IDXGISwapChain* swapChain);
	...
	// pos：描画ポジション
	// options：テキストの整形
	HRESULT DrawString(std::string str, D2D1_POINT_2F pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow = false);
	D2D1_SIZE_F MeasureString(const std::string& str);
	// 文字描画
	// string：文字列
	// rect：領域指定
	// options：テキストの整形
	...
	void SetTextColor(const D2D1_COLOR_F& color) { Setting.Color = color; if (pBrush) pBrush->SetColor(color); }

	D2D1_COLOR_F GetTextColor() const { return Setting.Color; }
...
private:
	...
	static std::vector<DirectWriteCustomFont*> instances;
};