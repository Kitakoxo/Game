...
void ed::Pin::Draw(ImDrawList* drawList, DrawFlags flags)
{
    ...
    drawList->ChannelsSetCurrent(m_Node->m_Channel + c_NodePinChannel);
    ImVec2 center = m_Pivot.GetCenter();
    float radius = ImMin(m_Bounds.GetWidth(), m_Bounds.GetHeight()) * 0.25f;
    drawList->AddCircleFilled(center, radius, IM_COL32(200, 200, 200, 255));
    drawList->AddCircle(center, radius, IM_COL32(0, 0, 0, 255));
}
